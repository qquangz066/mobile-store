        ...
        <div class=" card-header border-0">
          <div class="row align-items-center">
            <div class="col-lg-6 col-7">
              <h6 class="h2 d-inline-block mb-0">Brand Detail</h6>
            </div>
          </div>
        </div>
        <hr class="mt-2 mb-1"/>
  ...
  methods: {
    onSubmit() {
      schema.validate({...this.brand}, {abortEarly: false})
          .then(async () => {
            this.errors = {};
            this.brand = await this.$services.admin.brand.update(this.$route.params.id, this.brand)
    ...
    async getBrand() {
      this.brand = await this.$services.admin.brand.get(this.$route.params.id)
    },
